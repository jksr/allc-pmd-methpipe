...
static void
parse_site_old(const string &line, string &chrom, size_t &pos,
...
parse_site(const string &line, string &chrom, size_t &pos,
  ...
  std::istringstream iss;
  iss.rdbuf()->pubsetbuf(const_cast<char*>(line.c_str()), line.length());
  if (!(iss >> chrom >> pos >> strand >> seq >> mc >> coverage))
    throw std::runtime_error("bad allc site line: \"" + line + "\"");
  meth = mc/coverage;
  strand.resize(1);
  if (strand[0] != '-' && strand[0] != '+')
    throw std::runtime_error("bad methpipe site line, strand incorrect: \"" +
...
main(int argc, const char **argv) {
  try {
    ...
    const string description =
      "This is a modified version of methpipe-pmd (v4.1.1) to fit allc file format \
      (https://github.com/yupenghe/methylpy#output-format).\n\ 
      For the details of methpipe, see https://github.com/smithlabcode/methpipe\n\n\ 
